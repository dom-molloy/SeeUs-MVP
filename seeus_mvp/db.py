...
CREATE TABLE IF NOT EXISTS bugs (
    ...
    status TEXT,              -- New|In Progress|Fixed|Verified|Closed|Rejected
    assignee TEXT,
...
def init_db():
    with conn() as c:
        # Run schema statement-by-statement so we can identify the exact failing SQL.
        for i, stmt in enumerate(_split_sql_statements(SCHEMA_SQL), start=1):
            try:
                c.execute(stmt)
            except sqlite3.OperationalError as e:
                raise RuntimeError(
                    f"DB schema failed on statement #{i}\n\nSTATEMENT:\n{stmt}\n\nsqlite error: {e}"
                ) from e
