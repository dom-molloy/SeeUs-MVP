...
def render_memory(rid):
    ...
    with c1:
        ...
        for r in get_last_answers(rid, respondent="A", limit=6) or []:
            st.write(f"- {r['question_id']}: {r['answer_text'][:90]}")
...
with st.sidebar:
    st.header("SeeUs")
    page = st.radio("Go to", ["Assess", "Report"], index=0)
...
if forced_rid:
    ...
    if selected == "(new)":
        ...
        if st.button("Create"):
            ...
            create_relationship(new_rid, st.session_state["user_id"], other_id.strip() or None, label.strip() or "Untitled")
...
if not forced_rid:
    with st.expander("Invite link (Duo mode)"):
        ...
        if st.button("Create invite link"):
            ...
            # Streamlit local URL varies; user can paste base URL and add ?t=...
            qs = urlencode({"t": t})
            st.code(f"?{qs}")
            st.caption("Copy that query string and append it to your app URL (e.g., https://yourapp.com/?t=TOKEN).")
    ...
    if rows_s and not rows_a and not rows_b:
        ...
        if st.button("Generate Deep Research Brief"):
            try:
                ...
                # Deltas (last 2 changes per question)
                ...
                # Use LLM dimension scores if present in session (we have `dim_scores` only in LLM path)
                # If we are in heuristic path, convert scores dict to list-like structure.
                dimension_scores = []
    ...
    if rows_a and rows_b:
        ...
        if use_llm:
            try:
                ...
                st.info("Tip: set OPENAI_API_KEY in your environment, e.g. `export OPENAI_API_KEY=...` then restart Streamlit.")
...
rows_all = get_answers_for_session(sid)
...
def _maybe_queue_branches(latest_answer_text: str, q_obj: dict):
    # Simple MVP triggers (safe, non-clinical):
    # - If answer is short/vague and there is a branch, queue it.
    # - If numeric closeness exists in duo later, queue gap follow-up (handled in report/deep research too).
...
# Mirror Moment: after first 5 primary answers, once per respondent per session
...
if (not st.session_state[mm_key]) and primary_done >= 5:
    ...
    # Minimal MVP mirror synthesis (no LLM needed)
    # Strength: values clarity (length), Tension: closeness number presence, Cost: cost tolerance present
...
with c1:
    if st.button("Save & next"):
        save_response(
            ...
            answer_json=json.dumps({"dimension": q["dimension"]}),
        )
        st.rerun()
with c2:
    if st.button("Skip"):
        save_response(
            ...
            question_id=q["id"],
            answer_text="",
            answer_json=json.dumps({"skipped": True, "dimension": q["dimension"]}),
        )
        st.rerun()


elif page == "Growth":
    if "relationship_id" not in st.session_state:
        st.info("Start or select a relationship in **Assess** first.")
    else:
        rid = st.session_state["relationship_id"]
        sess_mode = st.session_state.get("mode", "solo")
        if sess_mode == "duo":
